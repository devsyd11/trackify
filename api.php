...
require_once $baseDir . '/includes/ig_monitor_helpers.php';
...
switch ($action) {
    ...
    case 'fb_monitor_debug':
        ...
        break;
    case 'ig_monitor_list':
        handleIgMonitorList();
        break;
    case 'ig_monitor_add':
        handleIgMonitorAdd();
        break;
    case 'ig_monitor_remove':
        handleIgMonitorRemove();
        break;
    case 'ig_monitor_check':
        handleIgMonitorCheck();
        break;
    case 'ig_monitor_logs':
        handleIgMonitorLogs();
        break;
    default:
...
// ---------------------------------------------------------------------------
// Instagram Monitor handlers
// ---------------------------------------------------------------------------

function handleIgMonitorList(): void
{
    $uid = dashboard_session_user_id();
    $pdo = trackify_pdo();
    if (!$pdo instanceof PDO) {
    ...
    try {
        ...
        $params = [':uid' => $uid];
        if ($q !== '') {
            $like = '%' . ig_monitor_sql_like_escape($q) . '%';
            $where .= ' AND (profile_url LIKE :like1 OR label LIKE :like2)';
            $params[':like1'] = $like;
            $params[':like2'] = $like;
        }

        $countStmt = $pdo->prepare("SELECT COUNT(*) FROM instagram_monitor WHERE $where");
        $countStmt->execute($params);
        $total = (int) $countStmt->fetchColumn();
        ...
        $selectSql = "SELECT id, profile_url, label, last_status, last_checked_at, last_changed_at, created_at
             FROM instagram_monitor WHERE $where
             ORDER BY created_at DESC LIMIT $lim OFFSET $off";
        $stmt = $pdo->prepare($selectSql);
        $stmt->execute($params);
        $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        echo json_encode(['status' => 'error', 'message' => 'DB error — run the instagram_monitor migration: ' . $e->getMessage()]);
    }
}

function handleIgMonitorAdd(): void
{
    if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
        echo json_encode(['status' => 'error', 'message' => 'POST required']);
        return;
    ...
    $host = strtolower((string) parse_url($url, PHP_URL_HOST));
    if (strpos($host, 'instagram.com') === false && $host !== 'instagr.am') {
        echo json_encode(['status' => 'error', 'message' => 'Only instagram.com (or instagr.am) URLs are allowed.']);
        return;
    }

    $url = ig_normalize_profile_url($url);
    ...
    try {
        $countStmt = $pdo->prepare('SELECT COUNT(*) FROM instagram_monitor WHERE user_id = ?');
        $countStmt->execute([$uid]);
        if ((int) $countStmt->fetchColumn() >= 20) {
            echo json_encode(['status' => 'error', 'message' => 'Maximum 20 Instagram monitors per account.']);
            return;
        }
        $dupStmt = $pdo->prepare('SELECT profile_url FROM instagram_monitor WHERE user_id = ?');
        $dupStmt->execute([$uid]);
        foreach ($dupStmt->fetchAll(PDO::FETCH_COLUMN) as $existing) {
            if (ig_monitor_urls_equivalent((string) $existing, $url)) {
                echo json_encode(['status' => 'error', 'message' => 'This profile URL is already in your list.']);
                return;
            }
        }
        $ins = $pdo->prepare(
            'INSERT INTO instagram_monitor (user_id, profile_url, label, last_status, created_at)
             VALUES (?, ?, ?, \'unknown\', NOW())'
        );
        $ins->execute([$uid, $url, $label]);
        echo json_encode(['status' => 'success', 'id' => (int) $pdo->lastInsertId()]);
...
function handleIgMonitorRemove(): void
{
    if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
        echo json_encode(['status' => 'error', 'message' => 'POST required']);
        return;
    ...
    try {
        $sel = $pdo->prepare('SELECT profile_url FROM instagram_monitor WHERE id = ? AND user_id = ?');
        $sel->execute([$id, $uid]);
        $row = $sel->fetch(PDO::FETCH_ASSOC);
        $profileUrl = is_array($row) ? trim((string) ($row['profile_url'] ?? '')) : '';

        $del = $pdo->prepare('DELETE FROM instagram_monitor WHERE id = ? AND user_id = ?');
        $del->execute([$id, $uid]);
        if ($del->rowCount() > 0 && $profileUrl !== '') {
            ig_monitor_purge_activity_for_profile_urls($uid, [$profileUrl]);
        }
    } catch (PDOException $e) {
        echo json_encode(['status' => 'error', 'message' => 'DB error: ' . $e->getMessage()]);
...
function handleIgMonitorCheck(): void
{
    if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
        echo json_encode(['status' => 'error', 'message' => 'POST required']);
        return;
    ...
    try {
        if ($onlyId !== null) {
            $stmt = $pdo->prepare('SELECT * FROM instagram_monitor WHERE id = ? AND user_id = ?');
            $stmt->execute([$onlyId, $uid]);
        } else {
            $stmt = $pdo->prepare('SELECT * FROM instagram_monitor WHERE user_id = ?');
            $stmt->execute([$uid]);
        }
        $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
    } catch (PDOException $e) {
        echo json_encode(['status' => 'error', 'message' => 'DB error: ' . $e->getMessage() . '. Run the instagram_monitor CREATE TABLE migration.']);
        return;
    }
    ...
    foreach ($rows as $row) {
        $prevStatus = (string) $row['last_status'];
        $check      = ig_check_profile_url((string) $row['profile_url']);
        $newStatus  = $check['status'];
        $detail     = $check['detail'];
        $rowLabel   = (string) ($row['label'] ?? '');
        $profileUrl = (string) $row['profile_url'];
        ...
        if ($newStatus === 'unknown' && empty($check['update_last_status'])) {
            $results[] = ['id' => (int) $row['id'], 'status' => 'unknown', 'detail' => $detail, 'changed' => false];
            ig_monitor_append_activity($uid, 'dashboard', 'unknown', $detail, $rowLabel, $profileUrl);
            continue;
        }
        ...
        try {
            ...
            $results[] = ['id' => (int) $row['id'], 'status' => 'error', 'detail' => $e->getMessage(), 'changed' => false];
            ig_monitor_append_activity($uid, 'dashboard', 'error', $e->getMessage(), $rowLabel, $profileUrl);
            continue;
        }

        if ($changed && $newStatus === 'active'
                && in_array($prevStatus, ['inactive', 'unavailable', 'unknown'], true)) {
            ig_monitor_send_active_alert($profileUrl, $rowLabel);
        }

        $results[] = ['id' => (int) $row['id'], 'status' => $newStatus, 'detail' => $detail, 'changed' => $changed];
        ig_monitor_append_activity($uid, 'dashboard', $newStatus, $detail, $rowLabel, $profileUrl);
    }
...
function handleIgMonitorLogs(): void
{
    if ($_SERVER['REQUEST_METHOD'] !== 'GET') {
        echo json_encode(['status' => 'error', 'message' => 'GET required']);
        return;
    ...
    $uid = dashboard_session_user_id();
    $entries = ig_monitor_read_activity_log($uid);
    ...
    if ($monitorId > 0) {
        ...
        try {
            $stmt = $pdo->prepare('SELECT profile_url, label FROM instagram_monitor WHERE id = ? AND user_id = ? LIMIT 1');
            $stmt->execute([$monitorId, $uid]);
            $row = $stmt->fetch(PDO::FETCH_ASSOC);
        } catch (PDOException $e) {
            echo json_encode(['status' => 'error', 'message' => 'DB error']);
        ...
        foreach ($entries as $e) {
            ...
            $u = (string) ($e['profile_url'] ?? '');
            if (ig_monitor_urls_equivalent($u, $wantUrl)) {
                $filtered[] = $e;
            }
        }
        echo json_encode([
